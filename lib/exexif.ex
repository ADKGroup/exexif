defmodule Exexif do
  ...
  def read_tags(_, _, _, _, result) do
    Enum.into(result, %{})
  end
  ...
  defp extract_thumbnail(%{exif: exif} = result) do
    ...
    result =
      if Enum.all?(Exexif.Data.Thumbnail.fields(), fn e -> Enum.any?(exif_keys, &(&1 == e)) end) do
        Map.put(
          result,
          :thumbnail,
          struct(
    ...
    %{result | exif: Map.drop(exif, Exexif.Data.Thumbnail.fields())}
  end
